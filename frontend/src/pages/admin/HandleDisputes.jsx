...
export default function HandleDisputes() {
  ...
  useEffect(() => {
    api.get('/admin/disputes').then(res => setDisputes(res.data.data ?? res.data)).finally(()=>setLoading(false))
  }, [])
  ...
  const card = { background:'#161525',borderRadius:'14px',border:'1px solid #2a2940',padding:'20px',marginBottom:'16px' }
        ...
        : disputes.map(d=>(
          <div key={d.id} style={card}>
            ...
            <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'16px',marginBottom:'16px' }}>
              {/* Worker side */}
              <div style={{ background:'rgba(34,197,94,.06)',border:'1px solid rgba(34,197,94,.2)',borderRadius:'10px',padding:'14px' }}>
              ...
              {/* Employer side */}
              <div style={{ background:'rgba(251,191,36,.06)',border:'1px solid rgba(251,191,36,.2)',borderRadius:'10px',padding:'14px' }}>
                <div style={{ display:'flex',alignItems:'center',gap:'10px',marginBottom:'8px' }}>
