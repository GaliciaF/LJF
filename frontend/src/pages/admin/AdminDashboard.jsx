...
export default function AdminDashboard() {
  ...
  const handleVerify = async (id) => {
    await api.patch(`/admin/verifications/${id}`, { action: 'approve' })
    setVerifications(p => p.filter(v => v.id !== id))
  }
  const handleReject = async (id) => {
    await api.patch(`/admin/verifications/${id}`, { action: 'reject', rejection_reason: 'ID unclear' })
    setVerifications(p => p.filter(v => v.id !== id))
  }
  ...
  return (
    <div style={{ padding:'28px',maxWidth:'1280px' }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'24px' }}>
        {[
          { n: analytics?.total_users ?? '—',              l:'Total Users',            c:'↑ 18 new this week', accent:'#7c3aed', nc:'#a78bfa', ico:'👥' },
          { n: analytics?.total_jobs  ?? '—',              l:'Active Job Posts',       c:'↑ 8 posted today',   accent:'#22c55e', nc:'#22c55e', ico:'📋' },
          { n: analytics?.open_reports ?? '—',             l:'Open Reports',           c:'Needs review!',      accent:'#f87171', nc:'#f87171', ico:'🚨' },
          { n: analytics?.pending_verifications ?? '—',    l:'Pending Verifications',  c:'Queue growing',      accent:'#fbbf24', nc:'#fbbf24', ico:'🪪' },
        ].map(({ n,l,c,accent,nc,ico }) => (
        ...
        <div style={card}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'12px',marginTop:'16px',borderTop:'1px solid #2a2940',paddingTop:'16px' }}>
            {[
              ...
              <div key={l}><div style={{ fontSize:'22px',fontWeight:800,color:c }}>{n}</div><div style={{ fontSize:'11px',color:'#8b8aad' }}>{l}</div></div>
            ))}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={{ display:'flex',flexDirection:'column',gap:'10px' }}>
            {verifications.length === 0
              ? <div style={{ color:'#5a5978',fontSize:'13px' }}>No pending verifications.</div>
              : verifications.map(v => (
                ...
                : reports.map(r => {
                  ...
                  return (
                    <tr key={r.id}>
                      ...
                      <td style={td}>
                        <span style={tag(isRed?'rgba(248,113,113,.12)':'rgba(251,191,36,.12)',isRed?'#f87171':'#fbbf24',isRed?'rgba(248,113,113,.3)':'rgba(251,191,36,.3)')}>{r.reason}</span>
                      </td>
                      <td style={td}>
                        <div style={{ display:'flex',gap:'6px' }}>
