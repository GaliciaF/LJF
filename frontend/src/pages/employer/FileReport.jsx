...
export default function FileReport() {
  ...
  useEffect(() => {
    api.get('/employer/jobs', { params: { status:'done' } })
      .then(res => {
        const seen = new Map()
  ...
  const card = { background:'#fff',borderRadius:'14px',border:'1px solid #e5e0d0',padding: isMobile ? '16px' : '24px',boxShadow:'0 1px 3px rgba(0,0,0,.08)' }
  ...
  if (saved) return (
    <div style={{ padding: isMobile ? '14px' : '28px', maxWidth:'700px' }}>
  ...
  return (
    <div style={{ padding: isMobile ? '14px' : '28px', maxWidth:'700px', background:'#fffdf5', minHeight:'100vh' }}>
      ...
      {err && (
        <div style={{ background:'rgba(239,68,68,.1)',border:'1px solid rgba(239,68,68,.3)',borderRadius:'10px',padding:'12px 16px',marginBottom:'16px',color:'#ef4444',fontSize:'13px' }}>
          {err}
        </div>
      )}
      <div style={card}>
        ...
        <div style={{ marginBottom:'16px' }}>
          ...
          <select value={form.reported_id} onChange={e=>setForm(f=>({...f,reported_id:e.target.value}))} style={{ ...inp,appearance:'none' }}>
            ...
            {workers.map(w => (
              <option key={w.id} value={w.id}>{w.name}{w.worker_profile?.barangay ? ` · ${w.worker_profile.barangay}` : ''}</option>
            ))}
          </select>
          {workers.length === 0 && <div style={{ fontSize:'11px',color:'#9ca3af',marginTop:'4px' }}>Only workers from completed jobs will appear here.</div>}
        </div>
          ...
          <div style={{
            ...
            {REASONS.map((r, i) => (
              <div key={r} onClick={() => setSelectedReason(i)}
                style={{ display:'flex',alignItems:'center',gap:'12px',padding:'12px 14px',border:`1.5px solid ${selectedReason===i?'#d97706':'#e5e0d0'}`,borderRadius:'10px',cursor:'pointer',background:selectedReason===i?'rgba(217,119,6,.06)':'#fff' }}>
