...
export default function MyJob() {
  ...
  const [markingDone, setMarkingDone] = useState(null) // tracks which job is being marked
  ...
  const handleMarkDone = async (id) => {
    if (!confirm('Mark this job as Done? Workers will no longer see it.')) return
    setMarkingDone(id)
    try {
      await api.put(`/employer/jobs/${id}`, { status: 'done' })
      // Update job status in state first, THEN switch filter
      setJobs(prev => {
        const updated = prev.map(j => j.id === id ? { ...j, status: 'done' } : j)
        return updated
      })
      setFilter('done')
      flash('success', 'Job marked as done.')
    } catch {
      flash('error', 'Failed to mark job as done. Please try again.')
    } finally {
      setMarkingDone(null)
    }
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      flash('success', editing._reopening ? 'Job reposted and now available!' : 'Job updated.')
      setEditing(null)
      // If reopened, switch to available tab
          ...
          <div style={{ background:'#fff', borderRadius:'18px', padding:'28px', width:'100%', maxWidth:'620px', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 24px 64px rgba(0,0,0,.2)' }}>
            ...
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'16px' }}>
              <div style={{ fontSize:'17px', fontWeight:800 }}>
                {editing._reopening ? '🔄 Reopen Job Post' : '✏️ Edit Job Post'}
              </div>
            ...
            {editing._reopening && (
              <div style={{ background:'rgba(217,119,6,.08)', border:'1px solid rgba(217,119,6,.25)', borderRadius:'10px', padding:'12px 14px', marginBottom:'16px', fontSize:'13px', color:'#92400e', lineHeight:1.5 }}>
                📢 Review and update the details below — especially the start date. Click <b>Post Again</b> when ready and workers will see it immediately.
              </div>
            )}
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
                <div>
                  <label style={lbl}>
                    Start Date {editing._reopening && <span style={{ color:'#d97706', fontWeight:700 }}>*</span>}
                  </label>
                  <input type="date" value={editForm.start_date} onChange={e => setEditForm(f => ({ ...f, start_date:e.target.value }))}
                    style={{ ...inp, borderColor: editing._reopening && !editForm.start_date ? '#d97706' : '#e5e0d0' }} />
                  {editing._reopening && !editForm.start_date && (
                    <div style={{ fontSize:'11px', color:'#d97706', marginTop:'3px' }}>Please set a new start date</div>
                  )}
                </div>
                <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'20px', flexWrap:'wrap', gap:'10px' }}>
        <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
          {['all','available','not_available','done'].map(f => {
            const count = f === 'all' ? null : jobs.filter(j => j.status === f).length
            return (
              <div key={f} style={chip(filter === f)} onClick={() => setFilter(f)}>
                {f === 'all' ? 'All' : STATUS[f]?.label ?? f}
                {f !== 'all' && (
                  <span style={{ marginLeft:'5px', fontSize:'11px', opacity:.7 }}>({count})</span>
                )}
              </div>
            )
          })}
      ...
      {filtered.length === 0 ? (
        <div style={{ ...card, textAlign:'center', color:'#6b7280', padding:'48px' }}>
          ...
          <div style={{ fontWeight:700, marginBottom:'8px' }}>
            {filter === 'done' ? 'No completed jobs yet' : `No ${filter === 'all' ? '' : STATUS[filter]?.label?.toLowerCase()} jobs`}
          </div>
          {filter === 'all' && (
        ...
        <div key={j.id} style={{ ...card, opacity: j.status === 'done' ? .85 : 1 }}
          ...
          <div style={{ display:'flex', alignItems:'flex-start', gap:'14px' }}>
            <div style={{ fontSize:'38px', lineHeight:1, filter: j.status === 'done' ? 'grayscale(1)' : '' }}>{j.category?.emoji ?? '📋'}</div>
            <div style={{ flex:1 }}>
              <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'6px', flexWrap:'wrap' }}>
                <div style={{ fontWeight:700, fontSize:'16px', textDecoration: j.status === 'done' ? 'line-through' : 'none', color: j.status === 'done' ? '#9ca3af' : '#111827' }}>{j.title}</div>
              ...
              <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
                ...
                {/* Mark as Done — only for not_available */}
                {j.status === 'not_available' && (
                  <button
                    disabled={markingDone === j.id}
                    style={{ ...btn('rgba(22,163,74,.1)','#16a34a','rgba(22,163,74,.3)'), opacity: markingDone === j.id ? .6 : 1 }}
                    onClick={e => { e.stopPropagation(); handleMarkDone(j.id) }}>
                    {markingDone === j.id ? '⏳ Marking...' : '✅ Mark as Done'}
                  </button>
                )}

                {/* Reopen — for not_available AND done */}
                {(j.status === 'not_available' || j.status === 'done') && (
                  <button style={btn('transparent','#6b7280','#e5e0d0')}
                    onClick={e => { e.stopPropagation(); handleReopen(j) }}>
                    🔄 Reopen & Edit
                  </button>
                )}
