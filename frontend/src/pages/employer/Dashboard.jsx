...
export default function EmployerDashboard() {
  ...
  useEffect(() => {
    Promise.all([api.get('/employer/jobs'), api.get('/employer/profile')])
      .then(([jRes, pRes]) => { setJobs(jRes.data); setProfile(pRes.data.employer_profile) })
      .finally(() => setLoading(false))
  }, [])

  const handleAccept  = async (appId, jobId) => { await api.patch(`/employer/applications/${appId}`, { status:'accepted' }); navigate(`/employer/jobs`) }
  const handleDecline = async (appId)         => { await api.patch(`/employer/applications/${appId}`, { status:'declined' }) }

  const card = { background:'#fff',borderRadius:'14px',border:'1px solid #e5e0d0',padding:'24px',boxShadow:'0 1px 3px rgba(0,0,0,.08)' }
  const stat = (a) => ({ ...card,padding:'20px 24px',borderTop:`3px solid ${a}` })
  ...
  return (
    <div style={{ padding:'28px',maxWidth:'1280px',background:'#fffdf5',minHeight:'100vh' }}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'24px' }}>
        {[
          { n:activeJobs.length,        l:'Active Job Posts',  c:'Receiving applicants',   accent:'#d97706', nc:'#d97706' },
          { n:totalApplicants,          l:'New Applicants',    c:'Needs your review!',     accent:'#ef4444', nc:'#ef4444' },
          { n:jobs.filter(j=>j.status==='filled').length, l:'Workers Hired', c:'↑ 3 this month', accent:'#16a34a', nc:'#16a34a' },
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'24px' }}>
        ...
        <div>
          ...
          {activeJobs.length===0
            ? <div style={{ ...card,textAlign:'center',color:'#6b7280',padding:'32px' }}>No active jobs. <span style={{ color:'#d97706',cursor:'pointer' }} onClick={()=>navigate('/employer/create-job')}>Post one now →</span></div>
            : activeJobs.map(j=>(
              <div key={j.id} style={{ ...card,marginBottom:'12px',cursor:'pointer',transition:'all .2s' }}
                ...
                <div style={{ display:'flex',alignItems:'flex-start',gap:'12px' }}>
                  ...
                  {j.applications_count>0 && (
                    <button style={btn('#d97706','#fff')} onClick={e=>{e.stopPropagation();navigate('/employer/applicants')}}>Review →</button>
                  )}
                </div>
              </div>
            ))
        ...
        <div>
          ...
          <div style={card}>
            <div style={{ fontSize:'13px',color:'#6b7280',textAlign:'center',padding:'20px' }}>
              ...
              <span style={{ color:'#d97706',cursor:'pointer',fontWeight:600 }} onClick={()=>navigate('/employer/applicants')}>Go to Review Applicants →</span>
            </div>
          </div>
            ...
            <div style={card}>
              {jobs.filter(j=>j.start_date).length===0
                ...
                : jobs.filter(j=>j.start_date).slice(0,3).map(j=>(
                  <div key={j.id} style={{ background:'rgba(217,119,6,.04)',borderRadius:'10px',padding:'12px 14px',border:'1px solid rgba(217,119,6,.2)',marginBottom:'8px' }}>
