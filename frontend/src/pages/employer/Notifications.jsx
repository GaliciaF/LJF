...
const ICONS = { new_application:'👷', application_accepted:'✅', application_declined:'📋', new_message:'💬', job_done:'🏁', new_review:'⭐', user_reported:'🚨' }
...
export default function EmployerNotifications() {
  ...
  const markRead    = async (id) => { await api.patch(`/employer/notifications/${id}/read`).catch(()=>{}); setNotifications(p=>p.map(n=>n.id===id?{...n,read_at:new Date().toISOString()}:n)) }
  const markAllRead = async () => { await api.patch('/employer/notifications/read-all').catch(()=>{}); setNotifications(p=>p.map(n=>({...n,read_at:n.read_at??new Date().toISOString()}))) }
  ...
  return (
    <div style={{ padding: isMobile?'14px':'28px', maxWidth:'720px', background:'#fffdf5', minHeight:'100vh' }}>
      <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px',flexWrap:'wrap',gap:'10px' }}>
        <div style={{ display:'flex',alignItems:'center',gap:'10px' }}>
          <div style={{ fontSize: isMobile?'16px':'20px',fontWeight:800 }}>🔔 Notifications</div>
          {unreadCount>0 && <span style={{ background:'#d97706',color:'#fff',borderRadius:'20px',padding:'2px 10px',fontSize:'12px',fontWeight:700 }}>{unreadCount} new</span>}
        </div>
        {unreadCount>0 && (
      ...
      {filtered.length===0 ? (
        ...
        <div style={card}>
          {filtered.map((n,i) => {
            const data=n.data??{}, isUnread=!n.read_at
            return (
              <div key={n.id}
                style={{ display:'flex',gap: isMobile?'10px':'12px',padding: isMobile?'12px':'16px',borderBottom:i<filtered.length-1?'1px solid #f3f4f6':'none',cursor:'pointer',background:isUnread?'rgba(217,119,6,.03)':'transparent',transition:'background .15s',alignItems:'flex-start' }}
                ...
                <div style={{ display:'flex',flexDirection:'column',alignItems:'center',gap:'8px',flexShrink:0 }}>
                  ...
                  <button onClick={e=>{e.stopPropagation();deleteOne(n.id)}} style={{ background:'none',border:'none',color:'#d1d5db',cursor:'pointer',fontSize:'14px',padding:'0' }} title="Dismiss">✕</button>
                </div>
              </div>
            )
          })}
