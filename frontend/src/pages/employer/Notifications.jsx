...
export default function EmployerNotifications() {
  ...
  useEffect(() => {
    api.get('/employer/notifications').then(res => setNotifications(res.data)).catch(()=>setNotifications([])).finally(()=>setLoading(false))
  }, [])
  ...
  const Item = ({ n }) => (
    <div onClick={()=>markRead(n.id)} style={{ display:'flex',gap:'12px',padding:'14px 16px',borderBottom:'1px solid #f3f4f6',cursor:'pointer',background:!n.read_at?'rgba(217,119,6,.04)':'transparent',transition:'background .15s' }}
        ...
        : <>
          {unread.length>0 && (
            <div style={{ marginBottom:'20px' }}>
              <div style={{ fontSize:'12px',fontWeight:700,color:'#6b7280',letterSpacing:'1px',textTransform:'uppercase',marginBottom:'10px' }}>New ({unread.length})</div>
