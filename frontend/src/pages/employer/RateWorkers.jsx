...
export default function RateWorkers() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/employer/jobs', { params:{ status:'filled' } }),
      api.get('/employer/reviews'),
    ]).then(([jRes, rRes]) => { setJobs(jRes.data); setReviews(rRes.data) })
    ...
    <div style={{ padding:'28px',maxWidth:'800px',background:'#fffdf5',minHeight:'100vh' }}>
      ...
      <div style={card}>
        ...
        <div style={{ marginBottom:'14px' }}>
          <div style={{ fontSize:'12px',fontWeight:600,color:'#6b7280',marginBottom:'6px' }}>Select Job</div>
          <select value={form.job_id} onChange={e=>{
            const j = jobs.find(j=>j.id===parseInt(e.target.value))
            setForm(f=>({...f, job_id:e.target.value, reviewee_id: j?.hired_worker_id??'' }))
          }} style={inp}>
        ...
        <div style={{ marginBottom:'14px' }}>
          ...
          <div style={{ display:'flex',gap:'8px' }}>
            {[1,2,3,4,5].map(n=>(
              <span key={n} onClick={()=>setForm(f=>({...f,rating:n}))} style={{ fontSize:'28px',cursor:'pointer',filter:form.rating>=n?'none':'grayscale(1) opacity(.4)',transition:'filter .15s' }}>⭐</span>
            ))}
          </div>
        </div>
