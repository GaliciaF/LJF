...
export default function RateWorkers() {
  ...
  useEffect(() => {
    Promise.all([
      // ✅ FIX: get DONE jobs instead of "filled"
      api.get('/employer/jobs', { params:{ status:'done' } }),
      api.get('/employer/reviews'),
    ])
    .then(([jRes, rRes]) => {
      // ✅ FILTER: only jobs with hired workers
      const completedJobs = (jRes.data || []).filter(j => j.hired_worker)
  ...
  return (
    <div style={{ padding:'28px',maxWidth:'800px',background:'#fffdf5',minHeight:'100vh' }}>
      ...
      {saved && (
        <div style={{ background:'rgba(22,163,74,.1)',border:'1px solid rgba(22,163,74,.3)',borderRadius:'10px',padding:'12px 16px',marginBottom:'16px',color:'#16a34a',fontSize:'13px',fontWeight:500 }}>
          ✓ Review submitted!
        </div>
      )}
      ...
      <div style={card}>
          ...
          >
            ...
            {jobs.map(j=>(
              <option key={j.id} value={j.id}>
                {j.title} — {j.hired_worker?.name ?? 'Worker'}
              </option>
            ))}
          </select>
        </div>
            ...
            {[1,2,3,4,5].map(n=>(
              <span
                ...
                style={{
                  fontSize:'28px',
                  cursor:'pointer',
                  filter:form.rating>=n?'none':'grayscale(1) opacity(.4)'
                }}
              >
                ⭐
              </span>
        ...
        <button style={btn('#d97706','#fff')} onClick={handleSubmit}>
          Submit Review
        </button>
      </div>
          ...
          {reviews.map(r=>(
            <div key={r.id} style={{ padding:'12px 0',borderBottom:'1px solid #e5e0d0' }}>
              <div style={{ display:'flex',gap:'10px',alignItems:'center',marginBottom:'6px' }}>
                <div style={{ ...ava(32),background:'linear-gradient(135deg,#16a34a,#15803d)' }}>
                  {ini(r.reviewer?.name)}
                </div>
