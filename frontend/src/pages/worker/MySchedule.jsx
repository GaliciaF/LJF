        ...
        <div style={{ display:'flex',flexDirection:'column',gap:'16px' }}>
          ...
          <div style={card}>
            ...
            <div style={{ display:'flex',gap:'8px' }}>
              ...
              <button onClick={addBlock} style={{ background:'transparent',border:'1px solid #e2e8e2',padding:'10px 14px',borderRadius:'9px',fontSize:'13px',color:'#6b7280',cursor:'pointer' }}>+ Block</button>
            </div>
          </div>
          ...
          {jobs.length === 0
            ...
            : jobs.map(app=>(
              <div key={app.id} style={{ display:'flex',alignItems:'center',gap:'14px',padding:'14px 16px',border:'1px solid #e2e8e2',borderRadius:'14px',marginBottom:'10px' }}>
                <div style={{ fontSize:'28px' }}>{app.job.category?.emoji??'💼'}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontWeight:600,fontSize:'14px' }}>{app.job.title}</div>
                  <div style={{ fontSize:'12px',color:'#6b7280' }}>{app.job.employer?.employer_profile?.household_name ?? app.job.employer?.name} · {app.job.barangay}</div>
