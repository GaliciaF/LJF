...
export default function WorkerMessages() {
  ...
  useEffect(() => {
    api.get('/worker/messages')
      .then(r => {
        setConvos(r.data)
        if (r.data.length > 0 && !isMobile) openThread(r.data[0])
      })
  ...
  const AvatarCircle = ({ photo, name, size = 40 }) => (
    <div style={{ width:`${size}px`, height:`${size}px`, borderRadius:'50%', background:'linear-gradient(135deg,#f59e0b,#d97706)', display:'flex', alignItems:'center', justifyContent:'center', fontSize:`${size*.35}px`, fontWeight:700, color:'#fff', flexShrink:0, overflow:'hidden' }}>
      {photo
  ...
  if (isMobile && showThread && active) {
    return (
      <div style={{ display:'flex', flexDirection:'column', height:'calc(100vh - 130px)', background:'#f9fafb' }}>
        {/* Header */}
        <div style={{ padding:'12px 16px', borderBottom:'1px solid #e2e8e2', display:'flex', alignItems:'center', gap:'10px', background:'#fff', flexShrink:0 }}>
          <button onClick={() => setShowThread(false)} style={{ background:'none', border:'none', fontSize:'18px', cursor:'pointer', color:'#6b7280', padding:'0 4px' }}>←</button>
          <AvatarCircle photo={active.user.photo} name={active.user.name} size={34} />
          <div>
            ...
            return (
              <div key={m.id ?? i}>
                ...
                <div style={{ display:'flex', gap:'8px', flexDirection:isMe?'row-reverse':'row', alignItems:'flex-end' }}>
                  {!isMe && <AvatarCircle photo={active.user.photo} name={active.user.name} size={26} />}
        ...
        {/* Input */}
        <div style={{ padding:'12px 16px', borderTop:'1px solid #e2e8e2', display:'flex', gap:'10px', alignItems:'center', background:'#fff', flexShrink:0 }}>
          <input
          ...
          <button onClick={sendMessage} disabled={sending || !body.trim()}
            style={{ width:'40px', height:'40px', borderRadius:'50%', background:body.trim()?'#16a34a':'#e2e8e2', color:'#fff', border:'none', fontSize:'16px', cursor:body.trim()?'pointer':'default', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
            ➤
          </button>
        </div>
      </div>
  ...
  return (
    <div style={{ padding: isMobile ? '16px' : '28px', maxWidth:'1100px' }}>
      {convos.length === 0 ? (
        ...
        /* Mobile: list only */
        <div style={{ border:'1px solid #e2e8e2', borderRadius:'14px', overflow:'hidden', background:'#fff', boxShadow:'0 2px 8px rgba(0,0,0,.06)' }}>
          <div style={{ padding:'14px 16px', borderBottom:'1px solid #e2e8e2', fontWeight:700, fontSize:'15px' }}>
            💬 Messages
          ...
          {convos.map(convo => (
            <div key={convo.user.id} onClick={() => openThread(convo)}
              style={{ display:'flex', gap:'12px', alignItems:'center', padding:'14px 16px', borderBottom:'1px solid #f3f4f6', cursor:'pointer' }}>
              <AvatarCircle photo={convo.user.photo} name={convo.user.name} size={40} />
        ...
        <div style={{ display:'grid', gridTemplateColumns:'300px 1fr', height:'600px', border:'1px solid #e2e8e2', borderRadius:'14px', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,.06)' }}>
          ...
          <div style={{ background:'#fff', borderRight:'1px solid #e2e8e2', display:'flex', flexDirection:'column' }}>
            ...
            <div style={{ flex:1, overflowY:'auto' }}>
              {convos.map(convo => (
                <div key={convo.user.id} onClick={() => openThread(convo)}
                  style={{ display:'flex', gap:'12px', alignItems:'center', padding:'14px 16px', borderBottom:'1px solid #f3f4f6', cursor:'pointer', background:active?.user.id === convo.user.id ? 'rgba(22,163,74,.08)' : 'transparent' }}>
                  <AvatarCircle photo={convo.user.photo} name={convo.user.name} size={40} />
                  <div style={{ flex:1, minWidth:0 }}>
                    <div style={{ fontWeight:700, fontSize:'14px', color:'#111827' }}>{convo.user.name}</div>
                    <div style={{ fontSize:'12px', color:'#6b7280', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
            ...
            {active ? (
              <>
                <div style={{ padding:'14px 20px', borderBottom:'1px solid #e2e8e2', display:'flex', alignItems:'center', gap:'12px', background:'#fff' }}>
                  <AvatarCircle photo={active.user.photo} name={active.user.name} size={38} />
                  <div>
                    <div style={{ fontWeight:700, fontSize:'14px' }}>{active.user.name}</div>
                    <div style={{ fontSize:'11px', color:'#16a34a' }}>Employer</div>
                  </div>
                  ...
                  {thread.map((m, i) => {
                    ...
                    return (
                      <div key={m.id ?? i}>
                        ...
                        <div style={{ display:'flex', gap:'8px', flexDirection:isMe?'row-reverse':'row', alignItems:'flex-end' }}>
                          {!isMe && <AvatarCircle photo={active.user.photo} name={active.user.name} size={28} />}
                ...
                <div style={{ padding:'14px 16px', borderTop:'1px solid #e2e8e2', display:'flex', gap:'10px', alignItems:'center', background:'#fff' }}>
                  ...
                  />
                  <button onClick={sendMessage} disabled={sending || !body.trim()}
                    style={{ width:'40px', height:'40px', borderRadius:'50%', background:body.trim()?'#16a34a':'#e2e8e2', color:'#fff', border:'none', fontSize:'18px', cursor:body.trim()?'pointer':'default', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0 }}>
                    ➤
                  </button>
                </div>
              </>
