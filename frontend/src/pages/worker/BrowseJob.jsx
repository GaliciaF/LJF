...
export default function BrowseJob() {
  ...
  const submitApply = async () => {
    ...
    try {
      // Use FormData so we can attach the resume file
    ...
    <div style={{ padding:'28px', maxWidth:'1280px' }}>
          ...
          return (
            <div key={job.id} style={{ background:'#fff', borderRadius:'14px', border:'1px solid #e2e8e2', padding:'18px 20px', boxShadow:'0 1px 3px rgba(0,0,0,.08)', display:'flex', flexDirection:'column', gap:'10px' }}>
              ...
              <button
                ...
                disabled={isApplied}
                style={{ width:'100%', padding:'9px', borderRadius:'9px', border:'none', fontSize:'13px', fontWeight:600, cursor:isApplied?'default':'pointer', background:isApplied?'rgba(22,163,74,.1)':'#16a34a', color:isApplied?'#16a34a':'#fff' }}>
      ...
      {coverModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.45)', zIndex:999, display:'flex', alignItems:'center', justifyContent:'center', padding:'20px' }}>
          <div style={{ background:'#fff', borderRadius:'16px', padding:'28px', width:'100%', maxWidth:'480px', boxShadow:'0 20px 60px rgba(0,0,0,.25)' }}>
            ...
            <textarea
              ...
              style={{ width:'100%', padding:'10px 12px', border:'1.5px solid #e2e8e2', borderRadius:'9px', fontSize:'13px', resize:'vertical', outline:'none', boxSizing:'border-box', marginBottom:'16px' }}
            />
            ...
            <div
              ...
              style={{ border:`2px dashed ${resumeFile ? '#16a34a' : '#e2e8e2'}`, borderRadius:'10px', padding:'14px 16px', cursor:'pointer', display:'flex', alignItems:'center', gap:'12px', background: resumeFile ? 'rgba(22,163,74,.04)' : '#fafafa', marginBottom:'20px', transition:'border-color .2s' }}
              onMouseEnter={e => e.currentTarget.style.borderColor='#16a34a'}
              onMouseLeave={e => e.currentTarget.style.borderColor = resumeFile ? '#16a34a' : '#e2e8e2'}
            >
              <span style={{ fontSize:'24px' }}>{resumeFile ? '📄' : '📎'}</span>
              <div>
                {resumeFile ? (
                  <>
              ...
              {resumeFile && (
                <span
                  ...
                  style={{ marginLeft:'auto', fontSize:'18px', color:'#9ca3af', cursor:'pointer', lineHeight:1 }}
                  title="Remove file"
                >×</span>
              )}
            </div>
