  ...
  return (
    <div style={{ padding:'28px',maxWidth:'1280px' }}>
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:'16px',marginBottom:'24px' }}>
        {[
          { n:completed,    l:'Jobs Accepted',        c:`${completed} total`,   accent:'#16a34a', nc:'#16a34a' },
          { n:pending,      l:'Pending Applications', c:'Under review',         accent:'#3b82f6', nc:'#3b82f6' },
          { n:avgRating > 0 ? `${avgRating}★` : '—', l:'Your Rating', c:avgRating >= 4.5 ? 'Top Worker! 🏆' : 'Keep going!', accent:'#f59e0b', nc:'#f59e0b' },
          { n:`₱${(parseFloat(profile?.expected_rate)||0).toLocaleString()}`, l:'Your Rate', c:`per ${(profile?.rate_type??'Day').toLowerCase()}`, accent:'#7c3aed', nc:'#7c3aed' },
        ].map(({ n,l,c,accent,nc }) => (
        ...
        <div>
          ...
          {jobs.length === 0 ? (
            ...
            <div style={{ display:'flex',flexDirection:'column',gap:'12px' }}>
              {jobs.map(job => (
                <div key={job.id} style={{ ...card,cursor:'pointer' }} onClick={() => navigate('/worker/browse-job')}>
